import type { Metadata } from "next";

export const metadata: Metadata = { title: "Applications" };

const SAMPLE = [
  { id: "APP-2024-018", type: "NOC: Sale", applicant: "Ramesh Iyer", unit: "A-401", submitted: "10 Aug 2024", status: "Pending", stage: "Level 2 Approval", overdue: false },
  { id: "APP-2024-017", type: "Sub-let Permission", applicant: "Priya Menon", unit: "B-204", submitted: "04 Aug 2024", status: "Pending", stage: "Secretary Sign-off", overdue: true },
  { id: "APP-2024-016", type: "NOC: Renovation", applicant: "Suresh Nair", unit: "A-102", submitted: "01 Aug 2024", status: "Approved", stage: "Completed", overdue: false },
  { id: "APP-2024-015", type: "Name Transfer", applicant: "Meena Rao", unit: "B-301", submitted: "28 Jul 2024", status: "Rejected", stage: "Completed", overdue: false },
  { id: "APP-2024-014", type: "Sub-let Permission", applicant: "Deepak Patel", unit: "C-507", submitted: "20 Jul 2024", status: "Approved", stage: "Completed", overdue: false },
];

const statusStyle: Record<string, { bg: string; text: string; border: string }> = {
  Pending: { bg: "rgba(245,158,11,0.1)", text: "#F59E0B", border: "rgba(245,158,11,0.2)" },
  Approved: { bg: "rgba(16,185,129,0.1)", text: "#10B981", border: "rgba(16,185,129,0.2)" },
  Rejected: { bg: "rgba(239,68,68,0.1)", text: "#EF4444", border: "rgba(239,68,68,0.2)" },
};

const FALLBACK_STYLE = { bg: "rgba(107,114,128,0.1)", text: "#6B7280", border: "rgba(107,114,128,0.2)" };

export default function ApplicationsPage() {
  return (
    <div className="page-container">
      <div className="page-header">
        <div>
          <h1 className="font-headline-lg-mobile text-headline-lg-mobile md:font-headline-lg md:text-headline-lg text-text-primary">
            Applications
          </h1>
          <p className="font-body-sm text-body-sm mt-1" style={{ color: "#9CA3AF" }}>
            NOC requests, permissions, and transfer applications
          </p>
        </div>
        <button
          className="flex items-center gap-2 px-4 py-2 rounded text-sm font-medium"
          style={{ backgroundColor: "#10B981", color: "#fff" }}
        >
          <span className="material-symbols-outlined" style={{ fontSize: "18px" }}>add</span>
          New Application
        </button>
      </div>

      {/* Tab filters */}
      <div className="flex items-center gap-1 mb-6" style={{ borderBottom: "1px solid #333333" }}>
        {["All", "Pending", "Approved", "Rejected"].map((tab, i) => (
          <button
            key={tab}
            className="px-4 py-2.5 text-sm font-medium transition-colors"
            style={{
              color: i === 0 ? "#10B981" : "#9CA3AF",
              borderBottom: i === 0 ? "2px solid #10B981" : "2px solid transparent",
              background: "none",
              border: "none",
              borderBottomWidth: "2px",
              borderBottomStyle: "solid",
              borderBottomColor: i === 0 ? "#10B981" : "transparent",
              cursor: "pointer",
            }}
          >
            {tab}
          </button>
        ))}
      </div>

      <div className="queue-section">
        <table className="w-full" style={{ borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ backgroundColor: "#1c1b1b", borderBottom: "1px solid #333333" }}>
              {["App ID", "Type", "Applicant", "Unit", "Submitted", "Stage", "Status", "Actions"].map((h) => (
                <th key={h} className="font-label-md text-label-md text-left px-4 py-3" style={{ color: "#6B7280" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {SAMPLE.map((row, i) => {
              const ss = statusStyle[row.status] ?? FALLBACK_STYLE;
              return (
                <tr key={row.id} style={{ borderBottom: i < SAMPLE.length - 1 ? "1px solid #2a2a2a" : "none" }}>
                  <td className="px-4 py-3 font-mono" style={{ fontSize: "13px", color: "#10B981" }}>
                    {row.id}
                    {row.overdue && <span className="ml-2" style={{ color: "#EF4444", fontSize: "11px" }}>⚑ overdue</span>}
                  </td>
                  <td className="px-4 py-3 font-body-sm text-body-sm text-text-primary">{row.type}</td>
                  <td className="px-4 py-3 font-body-sm text-body-sm text-text-primary">{row.applicant}</td>
                  <td className="px-4 py-3 font-body-sm text-body-sm" style={{ color: "#9CA3AF" }}>{row.unit}</td>
                  <td className="px-4 py-3 font-body-sm text-body-sm" style={{ color: "#9CA3AF" }}>{row.submitted}</td>
                  <td className="px-4 py-3 font-body-sm text-body-sm" style={{ color: "#9CA3AF" }}>{row.stage}</td>
                  <td className="px-4 py-3">
                    <span className="font-label-md text-label-md px-2 py-0.5 rounded" style={{ backgroundColor: ss.bg, color: ss.text, border: `1px solid ${ss.border}` }}>{row.status}</span>
                  </td>
                  <td className="px-4 py-3">
                    <button className="material-symbols-outlined" style={{ fontSize: "18px", color: "#6B7280", background: "none", border: "none", cursor: "pointer" }}>open_in_new</button>
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
        <div className="px-4 py-3" style={{ borderTop: "1px solid #333333", backgroundColor: "#1c1b1b" }}>
          <p className="font-body-sm text-body-sm italic" style={{ color: "#6B7280" }}>
            Phase 0: illustrative data. Live application workflows arrive in Phase 2.
          </p>
        </div>
      </div>
    </div>
  );
}
