...
import { NewMaintenanceWorkOrderModal } from "@/components/modals/NewMaintenanceWorkOrderModal";
...
export function WorkOrdersClient({ workOrders, vendors }: { workOrders: WorkOrder[]; vendors: Vendor[] }) {
  ...
  return (
    <>
        ...
        <div className="page-header">
          <div>
            ...
            <p className="font-body-sm text-body-sm mt-1" style={{ color: "#9CA3AF" }}>
              Maintenance work orders tracked to completion
            </p>
          </div>
          <button
            ...
            <table className="w-full" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "#1c1b1b", borderBottom: "1px solid #333333" }}>
                  {["Title", "Priority", "Vendor", "Estimated cost", "Status", "Scheduled"].map((h) => (
                ...
                {workOrders.map((row, i) => {
                  ...
                  return (
                    <tr key={row.id} style={{ borderBottom: i < workOrders.length - 1 ? "1px solid #2a2a2a" : "none" }}>
                      ...
                      <td className="px-4 py-3 font-body-sm text-body-sm" style={{ color: "#9CA3AF" }}>{row.vendor_name ?? "Not assigned"}</td>
                      <td className="px-4 py-3 font-body-sm text-body-sm" style={{ color: "#9CA3AF" }}>
                        {row.estimated_cost != null ? `INR ${row.estimated_cost.toLocaleString("en-IN")}` : "—"}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 font-body-sm text-body-sm" style={{ color: "#9CA3AF" }}>
                        {row.scheduled_date ? new Date(row.scheduled_date).toLocaleDateString("en-IN", { day: "2-digit", month: "short" }) : "—"}
                      </td>
                    </tr>
                  );
                })}
      ...
      <NewMaintenanceWorkOrderModal open={modalOpen} onClose={() => setModalOpen(false)} vendors={vendors} />
    </>
  );
}
