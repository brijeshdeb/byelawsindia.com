 ...
 *
 * NOTE: This root-level file takes precedence over src/middleware.ts.
 * Both files must stay in sync until the root file is deleted via:
 *   git rm middleware.ts
 *
 * Rules:
 *   /           → unauthenticated users see the public marketing homepage
 *                 authenticated users redirect to /select-context
 *   /login      → pass through; authenticated users redirect to /dashboard
 *   all others  → require authentication; redirect to /login?next=pathname
 *
